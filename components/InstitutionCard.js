import React from 'react'
import Link from 'next/link'
import Rating from './Rating'

function InstitutionCard({ name, location, rating, handleSuggestChange }) {
    return (
        <div className='row mt-3' style={{ justifyContent: 'center' }}>
            <div className="card col-md-3 my-2 institution-card" style={{
                width: '55rem',
                flexDirection: 'row',
                border: '1px solid #dfdfdf'
            }}>
                <img src="data:image/png;base64,"
                    className="card-img-top"
                    style={{ flex: 0, borderTopRightRadius: 0 }}
                />
                <div className="card-body">
                    <h3 className="card-title">{name}</h3>
                    <p className="card-text my-2 py-1">{location}</p>

                    {/* Star diye - like fashionista */}
                    <Rating value={rating} text={`(${rating})`}></Rating>

                    {/* <Link href={`/institution/${uuid}/suggestion`}> */}
                    <div className="d-flex justify-content-end">
                        <button onClick={handleSuggestChange} className="btn btn-primary mt-3">Suggest Changes</button>
                    </div>
                    {/* </Link> */}
                </div>
            </div>
        </div>
    )
}

export default InstitutionCard
